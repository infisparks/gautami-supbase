...
interface FilterState {
  searchQuery: string
  filterType: "dateRange" | "month" | "year" | "all"
...
const DashboardPage: React.FC = () => {
  ...
  const [filters, setFilters] = useState<FilterState>({
    searchQuery: "",
    filterType: "all", // Set default to 'all'
    selectedMonth: format(new Date(), "yyyy-MM"),
    selectedYear: "", // Default to empty for all years
    startDate: "", // Default to empty for all dates
    endDate: "", // Default to empty for all dates
    recordType: "all",
  })
  ...
  const filteredAppointments = useMemo(() => {
    ...
    let tempAppointments: CombinedAppointment[] = []

    // Apply date/year range filters
    ...
    } else if (filters.filterType === "year" && filters.selectedYear) {
      const year = filters.selectedYear
      opdAppointments.forEach((app) => {
        if (new Date(app.date).getFullYear().toString() === year) {
          tempAppointments.push(app)
        }
      })
      ipdAppointments.forEach((app) => {
        if (new Date(app.admissionDate).getFullYear().toString() === year) {
          tempAppointments.push(app)
        }
      })
    } else if (filters.filterType === "all") {
      // If filterType is 'all', include all OPD and IPD appointments
      tempAppointments = [...opdAppointments, ...ipdAppointments]
    }

    // Apply record type filter
    if (filters.recordType === "IPD") {
      tempAppointments = tempAppointments.filter((app) => app.type === "IPD")
    }
  ...
  }, [filters.searchQuery, filters.filterType, filters.startDate, filters.endDate, filters.selectedYear, filters.recordType, opdAppointments, ipdAppointments, allPatientsInfo])

  // Statistics
  const statistics = useMemo(() => {
    // These now operate on the *filtered* appointments
    const totalOpdAmt = filteredAppointments.reduce((sum, a) => (a.type === "OPD" ? sum + (a.payment.totalPaid || 0) : sum), 0)
    const totalIpdDep = filteredAppointments.reduce((sum, a) => (a.type === "IPD" ? sum + a.totalDeposit : sum), 0)
    const totalIpdRef = filteredAppointments.reduce((sum, a) => (a.type === "IPD" ? sum + a.totalRefunds : sum), 0)
    const opdCash = filteredAppointments.reduce((sum, a) => (a.type === "OPD" ? sum + a.payment.cashAmount : sum), 0)
    const opdOnline = filteredAppointments.reduce((sum, a) => (a.type === "OPD" ? sum + a.payment.onlineAmount : sum), 0)
    const ipdCash = filteredAppointments.reduce(
      (sum, a) =>
        a.type === "IPD"
          ? sum +
            a.payments
    ...
    const ipdOnline = filteredAppointments.reduce(
      (sum, a) =>
        a.type === "IPD"
          ? sum +
            a.payments
    ...
    return {
      totalOpdCount: filteredAppointments.filter((app) => app.type === "OPD").length,
      totalOpdAmount: totalOpdAmt,
      totalIpdCount: filteredAppointments.filter((app) => app.type === "IPD").length,
      totalIpdAmount: totalIpdDep,
      overallIpdRefunds: totalIpdRef,
      totalOtCount: filteredAppointments.filter((app) => app.type === "OT").length,
      opdCash,
      opdOnline,
      ipdCash,
      ipdOnline,
  ...
  const doctorConsultations = useMemo(() => {
    ...
    filteredAppointments.forEach((a) => {
      if (a.type === "OPD" && a.modalities && Array.isArray(a.modalities)) {
        a.modalities
  ...
  const chartData = useMemo(() => {
    ...
    filteredAppointments.forEach((a) => {
      if (a.type === "OPD" && opdCounts[a.date] !== undefined) opdCounts[a.date]++
    })

    const ipdCounts: Record<string, number> = { [dayBeforeYesterday]: 0, [yesterday]: 0, [today]: 0 }
    filteredAppointments.forEach((a) => {
      if (a.type === "IPD" && ipdCounts[a.admissionDate] !== undefined) ipdCounts[a.admissionDate]++
    })
  ...
  const handleFilterChange = (upd: Partial<FilterState>) => {
    setFilters((p) => ({
      ...p,
      ...upd,
      // When a new filter type is selected, reset other relevant filters
      ...(upd.filterType === "dateRange" && { selectedYear: "", selectedMonth: "" }),
      ...(upd.filterType === "year" && { startDate: "", endDate: "", selectedMonth: "" }),
      ...(upd.filterType === "month" && { startDate: "", endDate: "", selectedYear: "" }),
    }))
  }
  ...
  const getFilterTitle = () => {
    if (filters.filterType === "dateRange" && filters.startDate && filters.endDate) {
      return `Date Range: ${format(new Date(filters.startDate), "MMM dd")} - ${format(
        new Date(filters.endDate),
        "MMM dd, yyyy",
      )}`
    } else if (filters.filterType === "month" && filters.selectedMonth) {
      return `Month: ${format(new Date(filters.selectedMonth + "-01"), "MMM, yyyy")}`
    } else if (filters.filterType === "year" && filters.selectedYear) {
              ...
              <div className="bg-white rounded-xl shadow-sm mb-6 p-6 border border-gray-100">
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-4">
                  <h2 className="text-lg font-semibold text-gray-800 flex items-center mb-4 lg:mb-0">
                    <Filter className="mr-2 h-5 w-5 text-sky-500" /> Date and Record Type Filters
                  </h2>
                  <button
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  {/* Show Only IPD Records Checkbox */}
                  <div className="flex items-center mt-6">
                    <input
                      type="checkbox"
                      id="showIpdOnly"
                      checked={filters.recordType === "IPD"}
                      onChange={(e) =>
                        handleFilterChange({
                          recordType: e.target.checked ? "IPD" : "all",
                        })
                      }
                      className="h-4 w-4 text-sky-600 focus:ring-sky-500 border-gray-300 rounded"
                    />
                    <label htmlFor="showIpdOnly" className="ml-2 block text-sm font-medium text-gray-700">
                      Show only IPD Records
                    </label>
                  </div>
                </div>
